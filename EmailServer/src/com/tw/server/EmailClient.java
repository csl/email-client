...
public class EmailClient extends JFrame {
	...
	public void initialize() {
		...
		ImageIcon image1 = new ImageIcon(cl.getResource(""));
	...
	public JTabbedPane getJTabbedPane() {
		if (jTabbedPane == null) {
			...
			jTabbedPane.addTab("寫新信", getJPanel());
			jTabbedPane.addTab("收件匣", getJPanel8());
	...
	public JButton getJButtonSend() {
		if (jButtonSend == null) {
			...
			jButtonSend.setToolTipText("傳送信件(S)");
			...
			jButtonSend.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent ae) {
					try {
						...
						JOptionPane.showMessageDialog((Component) null,
							"郵件無法傳送！", "警告", JOptionPane.WARNING_MESSAGE);
						ade.printStackTrace();
	...
	public JButton getJButtonClear() {
		if (jButtonClear == null) {
			...
			jButtonClear.setToolTipText("清除重寫");
	...
	private JButton getJButtonSubex() {
		if (jButtonSubex == null) {
			...
			jButtonSubex.setToolTipText("常用主旨");
	...
	private JButton getJButtonReceive() {
		if (jButtonReceive == null) {
			...
			jButtonReceive.setToolTipText("檢查新信(R)");
			jButtonReceive
					.addActionListener(new java.awt.event.ActionListener() {
						public void actionPerformed(java.awt.event.ActionEvent e) {
							try {
	...
	private JTabbedPane getJTabbedPane1() {
		if (jTabbedPane1 == null) {
			...
			jTabbedPane1.addTab("關鍵字", getJPanel9());
		}
		return jTabbedPane1;
	}
	...
	private JDialog getJDialogAbout() {
		if (jDialogAbout == null) {
			...
			jDialogAbout.setTitle("關於 EmailCLient");
	...
	private JButton getJButtonArrowLeft() {
		if (jButtonArrowLeft == null) {
			...
			jButtonArrowLeft.setToolTipText("上一封(P)");
	...
	private JButton getJButtonArrowRight() {
		if (jButtonArrowRight == null) {
			...
			jButtonArrowRight.setToolTipText("下一封(N)");
	...
	private JButton getJButtonAddFile() {
		if (jButtonAddFile == null) {
			...
			jButtonAddFile.setToolTipText("附加檔案");
			...
			jButtonAddFile.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if (result == file.APPROVE_OPTION) {
						...
						"選擇的文件", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
						if(confirmAcceptReject == JOptionPane.YES_OPTION){
							Send.fds = new FileDataSource(dir + "\\" + fileName);
							jLabelStatus.setText("附加的檔案為：" + dir + fileName);
	...
	private JButton getJButtonDelete() {
		if (jButtonDelete == null) {
			...
			jButtonDelete.setToolTipText("移除附件");
			jButtonDelete.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					if(Send.fds != null) {
						Send.fds = null;
						jLabelStatus.setText("已移除附加檔案！");
						jButtonDelete.setEnabled(false);
					}
				}
			});
	...
	private JMenuItem getJMenuItemCalculator() {
		if (jMenuItemCalculator == null) {
			...
			jMenuItemCalculator.setText("計算機(C)");
